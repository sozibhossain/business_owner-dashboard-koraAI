...
const tabs: Array<{ id: TabId; label: string; title: string; subtitle: string }> = [
  { id: "general", label: "General", title: "Settings", subtitle: "Manage account information, password, interface and dashboard preferences." },
  { id: "business", label: "Business", title: "Business Settings", subtitle: "Manage business profile, contact details, address, opening hours and branding." },
  { id: "team", label: "Team & Roles", title: "Team & Roles", subtitle: "Manage employees, roles, permissions and subscription seat usage." },
  { id: "notifications", label: "Notifications", title: "Notifications", subtitle: "Configure channels, event alerts and digest settings." },
  { id: "billing", label: "Billing & Subscription", title: "Billing & Subscription", subtitle: "Manage plan, payment methods, billing history and usage limits." },
  { id: "integrations", label: "Integrations", title: "Integrations", subtitle: "Connect calendars, communication tools, website services and future automations." },
  { id: "security", label: "Security", title: "Security", subtitle: "Manage authentication, recovery methods, sessions and security logs." },
  ...
  return (
    <div className="space-y-3 2xl:space-y-4">
      ...
      <div className="grid gap-3 lg:grid-cols-3 2xl:gap-4">
        ...
        <ToggleList title="Dashboard Preferences" items={["Analytics Widgets", "Upcoming Appointments", "Revenue Overview", "Task Overview", "Recent Activity Visibility"]} />
...
function Team({ employees = [] }: { employees?: EmployeeData[] } = {}) {
  ...
  return <div className="space-y-5"><Section title="Employee Management" subtitle="Invite, edit, remove and manage employees." action={<Button className="gap-2"><UserPlus className="h-4 w-4" />Invite Employee</Button>}><div className="grid gap-4 sm:grid-cols-4"><Metric icon={Users} value={String(teamCount)} label="Current Seats" /><Metric icon={Shield} value="1" label="Owner" color="text-emerald-300" /><Metric icon={Crown} value="1" label="Manager" /><Metric icon={Users} value="10" label="Maximum Seats" color="text-purple-300" /></div></Section><div className="grid gap-5 lg:grid-cols-[1.35fr_0.8fr]"><Section title="Team Members" subtitle="View and manage employee details."><div className="overflow-hidden rounded-xl border border-[#1e2d40]">{rows.map((employee, index) => { const name = employee.name || "Team Member"; const status = employee.status || (index === 3 ? "Invited" : "Active"); return <div key={employee._id || employee.id || name} className="grid grid-cols-[1.4fr_0.8fr_1fr_0.8fr_40px] items-center border-b border-[#1e2d40] px-4 py-3 text-sm last:border-0"><div className="flex items-center gap-3"><Avatar className="h-9 w-9"><AvatarFallback>{getInitials(name)}</AvatarFallback></Avatar><div><p className="font-medium text-gray-100">{name}</p><p className="text-xs text-gray-500">{employee.email || `${name.toLowerCase().replace(" ", ".")}@koraai.de`}</p></div></div><span className="w-fit rounded bg-blue-600/20 px-2 py-1 text-xs text-blue-300">{employee.role || (index === 0 ? "Owner" : index === 1 ? "Manager" : "Employee")}</span><span>{employee.department || (index < 2 ? "Management" : index === 2 ? "Operations" : "Support")}</span><span className={status.toLowerCase() === "invited" ? "text-amber-400" : "text-emerald-400"}>{status}</span><MoreVertical className="h-4 w-4 text-gray-500" /></div>; })}</div></Section><Section title="Employee Limits" subtitle="Seats according to your subscription."><div className="flex flex-col items-center"><Ring value={`${teamCount} / 10`} label="Seats" /><Button className="mt-5 w-full">Upgrade Plan</Button></div></Section></div><div className="grid gap-5 lg:grid-cols-[1.35fr_0.8fr]"><ToggleList title="Roles & Permissions" items={["Calendar Access", "Customers Access", "Accounting Access", "Website Access", "Requests Access", "Reports Access", "Settings Access", "Team Management"]} /><Section title="Team Activity" subtitle="Overview of your team activity."><Row icon={Users} title="Active Today" sub="Team members active today" right={<span className="text-2xl text-emerald-400">{activeCount}</span>} /><Row icon={UserPlus} title="Pending Invites" sub="Invitations waiting for response" right={<span className="text-2xl text-amber-400">{invitedCount}</span>} /><Row icon={Globe} title="Online Now" sub="Currently online" right={<span className="text-2xl text-emerald-400">{Math.min(activeCount, 2)}</span>} /></Section></div></div>;
}
...
function Integrations() {
  const groups = [["Calendar Integrations", ["Google Calendar", "Outlook Calendar", "Apple Calendar", "Microsoft Exchange"]], ["Communication Integrations", ["WhatsApp Business", "Gmail", "Outlook", "Twilio", "Zoom", "Microsoft Teams"]], ["Website Integrations", ["Domain Connection", "Google Analytics", "Meta Pixel", "Google Business Profile", "WordPress", "Webflow"]], ["Future Integrations", ["Stripe", "Zapier", "Meta", "Google Business Profile", "PayPal", "Klarna"]]] as const;
...
function Security({ onChangePassword }: { onChangePassword?: () => void } = {}) {
  return <div className="space-y-5"><div className="grid gap-5 lg:grid-cols-2"><Section title="Authentication" subtitle="Update password, 2FA and recovery methods."><div className="grid gap-4 sm:grid-cols-2"><Field label="Current Password" type="password" value="" /><Field label="New Password" type="password" value="" /><Field label="Confirm Password" type="password" value="" /></div><div className="mt-5 flex justify-end"><Button onClick={onChangePassword}>Update Password</Button></div></Section><Section title="Security Status" subtitle="Your overall account security."><div className="flex items-center gap-8"><Ring value="92%" label="Security Score" /><div className="space-y-3">{["Strong Password", "2FA Enabled", "Recovery Email Added", "Phone Verified"].map((item) => <p key={item} className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="h-4 w-4 text-emerald-400" />{item}</p>)}<p className="border-t border-[#1e2d40] pt-4 text-sm text-emerald-400">Status: Protected</p></div></div></Section></div><div className="grid gap-5 lg:grid-cols-2"><Section title="Two-Factor Authentication" subtitle="Add an extra layer of security to your account."><h3 className="mb-4 flex items-center gap-2 text-xl font-semibold text-emerald-400"><ShieldCheck className="h-6 w-6" />Enabled</h3><Row icon={Shield} title="Authenticator App" sub="Use an authenticator app to generate codes" right={null} /><Row icon={Monitor} title="SMS Verification" sub="Receive verification codes via SMS" right={null} /><Row icon={Lock} title="Recovery Methods" sub="Manage recovery email and backup codes" right={<Button variant="outline">Manage Recovery</Button>} /></Section><Section title="Session Management" subtitle="Devices and sessions currently signed in."><Row icon={Monitor} title="MacBook Pro" sub="macOS - Safari" right={<Button size="sm" variant="outline">Logout</Button>} /><Row icon={Smartphone} title="iPhone 16 Pro" sub="iOS - Safari" right={<Button size="sm" variant="outline">Logout</Button>} /><Row icon={Monitor} title="Windows Desktop" sub="Windows - Chrome" right={<Button size="sm" variant="outline">Logout</Button>} /><Button variant="destructive" className="mt-4">Log Out All Devices</Button></Section></div><div className="grid gap-5 lg:grid-cols-2"><Section title="Security Log" subtitle="Recent login and security events.">{["Login from Berlin", "Password changed", "2FA recovery code generated"].map((event, index) => <Row key={event} icon={ShieldCheck} title={event} sub={["Today, 10:32", "Yesterday, 14:08", "Jul 10, 2026"][index]} right={<span className="text-xs text-gray-500">View</span>} />)}</Section><Section title="Danger Zone" subtitle="These actions are permanent and cannot be undone." className="border-red-500/30 bg-red-950/15"><Row icon={Download} title="Download Account Data" sub="Download a copy of your data." right={<Button size="sm" variant="outline">Download</Button>} /><Row icon={Lock} title="Archive Account" sub="Temporarily archive this workspace." right={<Button size="sm" variant="outline">Archive</Button>} /><Row icon={Trash2} title="Delete Account" sub="Permanently delete your account and all data." right={<Button size="sm" variant="destructive">Delete</Button>} /></Section></div></div>;
}
...
function Advanced({ aiHistoryCount = 0 }: { aiHistoryCount?: number } = {}) {
  return <div className="grid gap-5 xl:grid-cols-2"><Section title="Data Management" subtitle="Export, import and back up workspace data."><Row icon={Upload} title="Export Workspace Data" sub="Export all workspace data to a file." /><Row icon={Download} title="Download Account Data" sub="Download a copy of all your account data." /><Row icon={Archive} title="Import Data" sub="Import data from external sources." /><Row icon={Database} title="Backup Settings" sub="Create a backup of configuration and preferences." /></Section><Section title="API Access" subtitle="Future developer access for API keys and webhooks."><Row icon={Database} title="API Keys" sub="Create and rotate API credentials." right={<span className="text-xs text-amber-400">Future</span>} /><Row icon={Globe} title="Webhooks" sub="Send events to external systems." right={<span className="text-xs text-amber-400">Future</span>} /><Row icon={FileText} title="Developer Access" sub="Manage developer-level integrations." right={<span className="text-xs text-amber-400">Future</span>} /></Section><Section title="Activity Logs" subtitle="System events, changes and user actions.">{["Settings updated", "Billing portal opened", "Team role changed", "Data export requested"].map((item, index) => <Row key={item} icon={Clock} title={item} sub={["Today, 10:32", "Yesterday, 16:40", "Jul 10, 2026", "Jul 08, 2026"][index]} right={<span className="text-xs text-gray-500">View</span>} />)}</Section><Section title="Workspace Health" subtitle="Everything is running smoothly." action={<span className="rounded bg-emerald-600/20 px-3 py-1 text-xs text-emerald-400">Excellent</span>}>{["Database", "AI Services", "Integrations", "Storage", "AI Records"].map((item, index) => <Pair key={item} label={item} value={["Healthy", "Online", "8 Connected", "24% Used", String(aiHistoryCount)][index]} success={index < 4} />)}<div className="mt-6 flex items-center gap-5"><Ring value="98" label="/100" /><div><h4 className="font-semibold text-white">Health Score</h4><p className="mt-1 text-sm text-gray-400">Your workspace is optimized and performing at peak efficiency.</p></div></div></Section><ToggleList title="Data & Privacy" items={["Usage Analytics", "Performance Analytics", "Crash Reports", "Product Improvements", "Personalized Recommendations"]} /><Section title="Account Management" subtitle="Ownership and account lifecycle actions."><Row icon={Users} title="Transfer Ownership" sub="Move ownership to another team member." right={<Button size="sm" variant="outline">Transfer</Button>} /><Row icon={Archive} title="Archive Account" sub="Archive account while keeping data available." right={<Button size="sm" variant="outline">Archive</Button>} /><Row icon={Trash2} title="Delete Account" sub="Permanently delete workspace and all data." right={<Button size="sm" variant="destructive">Delete</Button>} /></Section><ToggleList title="Default Preferences" items={["Auto Save", "Remember Filters", "Open Last Workspace", "Compact Mode", "Enable Beta Features"]} /><ToggleList title="Experimental Features" badge="Early Access" items={["AI Beta Features", "Experimental UI", "Upcoming Dashboard Widgets", "Smart Recommendations"]} /></div>;
}
...
export default function SettingsRedesign() {
  ...
  const profilePhotoPreview = useMemo(
    () => (profilePhotoFile ? URL.createObjectURL(profilePhotoFile) : ""),
    [profilePhotoFile]
  );
